...
function insertDocument(collection) {
    ...
    collection.insertMany([catalog24, catalogAdams], (error, result) => {
        if (error) {
            ...
            console.log(result);
            findOneAndModifyDocument(collection);
        }
    });
}

function findOneAndModifyDocument(collection) {
    collection.findAndModify({ journal: 'Adams Harry Potter' }, [
        ['_id', 'ascending'],
        ['edition', 'descending']
    ], { edition: '01-01-2017', journal: 'OracleMagazine' }, {
        new: true,
        w: 1
