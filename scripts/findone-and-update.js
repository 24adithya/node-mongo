...
function insertDocument(collection) {
    ...
    collection.insertMany([catalog24, catalogAdams], (error, result) => {
        if (error) {
            ...
            console.log(result);
            findOneAndUpdateDocument(collection);
        }
    });
}

function findOneAndUpdateDocument(collection) {

    var modifiedAdams = {
        "_id": 'Adams',
        "journal": 'Modified Adams Harry Potter',
        "publisher": 'Modified Adams publisher ',
        "edition": 'Modified 2017',
        "title": 'Adams Undertaker',
        "author": 'Adams Undertaker'
    };

    collection.findOneAndUpdate({ journal: 'Adams Harry Potter' }, { $set: { languages: "Spanish, English", edition: "2018" } }, { upsert: true }, (error, result) => {
        if (error) {
            console.log(error)
        } else {
            console.log(result);
